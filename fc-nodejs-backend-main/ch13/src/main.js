...
const OWNER = 'haeny-dev'
const REPO = 'fc-nodejs-backend'
...
/**
 * Pull Request를 모두 검사해서, 만약 너무 diff가 큰 풀 리퀘스트가 있으면 `too-big` 이라는 레이블을 붙인다.
 * - diff가 큰 : 100 lines
 */
program
  .command('check-prs')
...
program
  ...
  .action(async () => {
    ...
    // 2. bug 레이블이 있고, needs-screenshot 있는데, 스크린샷 있음 => needs-screenshot 제거
